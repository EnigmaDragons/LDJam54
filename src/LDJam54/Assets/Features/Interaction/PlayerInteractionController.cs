...
namespace Features.Interaction
{
    public class PlayerInteractionController : MonoBehaviour
    {
        ...
        private void CheckForPickupOrSetDown()
        {
            ...
            if (held.IsPresent && dropTarget.IsPresent && IsDropActionTriggered() && Mathf.Abs(Quaternion.Dot(dropTarget.Value.TopmostDropTarget.transform.rotation, Quaternion.identity)) <= 25 && !Physics.Raycast(dropTarget.Value.TopmostDropTarget.transform.position + Vector3.down * 0.1f, Vector3.up, out var _, held.Value.Bounds.size.y, interactableLayer))
            {
                SetObject();
                return;
            }
